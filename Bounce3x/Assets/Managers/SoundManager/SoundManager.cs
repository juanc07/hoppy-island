...
public class SoundManager : MonoBehaviour {
	...
	public void PlaySfx(SFX sfxName, float volume =1f, bool isLoop =false){
	...
	public void StopSfx(SFX sfxName){
		AudioSource audioSfx = SearchForPlayingAudioSource(sfxName);
		AudioClip clip = CheckCachedSFX(sfxName);
	...
	private AudioSource SearchForPlayingAudioSource(SFX clipName){
		int audioSourceCnt = extraSfx.Count;
		//Debug.Log("Searching playing sfx audioSourceCnt " + audioSourceCnt);

		AudioSource found = null;
		for(int index=0;index<audioSourceCnt;index++){
			if(extraSfx[index]!= null){
				//Debug.Log("Searching playing sfx " + extraSfx[index].clip.name + " search key " + clipName.ToString());
				if(extraSfx[index].isPlaying && extraSfx[index].clip.name.Equals(clipName.ToString(),StringComparison.Ordinal)){;
					found = extraSfx[index];
					break;
				}
			}
